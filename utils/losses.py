...
def free_energy_mse(fine, coarse, batch_size=1, k=4):
    '''
    Mean-squared error of free energies
    '''
    
    # Gibbs-sampled marginal distributions
    p_x = fine.p_v(n=batch_size, k=k).detach()
    p_x_hat = coarse.p_v(p_x, n=batch_size, k=k)
...
def free_energy_difference(fine, coarse, batch_size=1, k=4):
    '''
...
def marginals_cross_entropy(fine, coarse, batch_size=1, k=4):
    '''
    Cross-entropy of marginal distributions
    '''
    
    # Gibbs-sampled marginals
    p_x = fine.p_v(n=batch_size, k=k)
    p_x_hat = coarse.p_v(p_x, n=batch_size, k=k)
