...
def free_energy_mse(fine, coarse, n_batches=1, k=4):
    '''
    Mean-squared error of free energies
    '''
    
    # Gibbs-sampled marginal distributions
    p_x = fine.p_v(n=n_batches, k=k).detach()
    p_x_hat = coarse.p_v(p_x, n=n_batches, k=k)
...
def free_energy_difference(fine, coarse, n_batches=1, k=4):
    '''
...
def marginals_cross_entropy(fine, coarse, n_batches=1, k=4):
    '''
    Cross-entropy of marginal distributions
    '''
    
    # Gibbs-sampled marginals
    p_x = fine.p_v(n=n_batches, k=k)
    p_x_hat = coarse.p_v(p_x, n=n_batches, k=k)
